...
double h; ///Esto no puede estar como variable global
double m;
...
vector <vector<int>> gridCreation(GridSize gridSize){
...
int find_block(Particle particle,GridSize gridSize){
    ...
    if (block_x < 0){
        block_x = 0;
    } else if (block_x >= gridSize.nx-1) {
        block_x = gridSize.nx-1;
    }if (block_y < 0){
        block_y = 0;
    } else if (block_y >= gridSize.ny-1) {
        block_y = gridSize.ny-1;
    }if (block_z < 0){
        block_z = 0;
    } else if (block_z >= gridSize.nz-1) {
        block_z = gridSize.nz-1;
    }
    //cout << "This is the x block " << block_x << ", y block " << block_y << ", z block " << block_z;
    int num_block = block_x + block_y*gridSize.nx + block_z*gridSize.ny*gridSize.nx;
    return num_block;
}

std::vector<int> get_contiguous_blocks(int current_block, GridSize gsize){
    std::vector<int> contiguous_blocks;
    int bz = current_block / (gsize.ny*gsize.nx);
    int by = (current_block - bz*(gsize.ny*gsize.nx)) / gsize.nx;
    int bx = current_block - (bz*gsize.ny*gsize.nx) - (by*gsize.nx);
                ...
                if ( (by + y>=0) && (by + y <= gsize.ny-1)) {
                    for (int z = -1; z < 2; z++) {
                        if ( (bz + z>=0) && (bz + z <= gsize.nz-1)){
                            int computed_block = (bx + x) + (by + y)*gsize.nx + (bz+z)*gsize.ny*gsize.nx;
...
void Z_boundary_interaction(std::vector<Particle> &particles, Grid &grid) {
    ...
    for (int i = 0; i < grid.size.nx*grid.size.ny; i++) {   //pared x_0 //the number of particles in the x axis is ny * nz twice x min and xmax
        for (int j: grid.blocks[i]) {
            distance_z = particles[j].pz - bmin[2];
            if (distance_z < 0) {
    ...
    for (int i = (grid.size.nz*grid.size.ny*grid.size.nx - grid.size.nx*grid.size.ny)-1; i < grid.size.nz*grid.size.ny*grid.size.nx; i++){    //pared x_max
        for (int j : grid.blocks[i]) {
            distance_z = bmax[2] - particles[j].pz;
            if (distance_z < 0) {
                particles[j].pz = bmax[2] + distance_z;
                particles[j].vz = -particles[j].vz;
                particles[j].hvz = -particles[j].hvz;
            }
        }
...
void Y_boundary_interaction(std::vector<Particle> &particles,  Grid &grid) {
    ...
    for (int i = 0; i < grid.size.nz; i++){     //the number of particles in the y axis is nx * nz, twice y min and xmax
        for (int j = 0, k=grid.size.nx*(grid.size.ny-1); j < grid.size.nx; j++,k++) {//pared Y_0
            ...
            for (int m : grid.blocks[k + i * grid.size.nx * grid.size.ny]) { //  block_index = k + i * grid.size.nz * grid.size.ny;
                distance_y = bmax[1] - particles[m].py;
                if (distance_y < 0) {
                    particles[m].py = bmax[1] + distance_y;
                    particles[m].vy = -particles[m].vy;
                    particles[m].hvy = -particles[m].hvy;
                }
            }
        }
    }
...
Grid grid_initialization(Initial_values &initialValues,vector<Particle> &particles){
    ...
    std::vector<vector <int>> blocks = gridCreation(gridSize);
    Grid grid;
    grid.size = gridSize;
    grid.blocks = blocks;
...
// TRAZE  BOUNDARY ITERATION
    ...
    load_trace("../trz/small/boundint-base-1.trz",grid,myparticles,densities,accelerations,initialValues);
    int num_block = 0;
    Particle mypart101;
    mypart101= myparticles[101];
    num_block = find_block( mypart101, grid.size); // ver que la particula 101 en el traze pertence al bloque 1 pero para nosotros al 315
    boundary_collision (myparticles,grid);
    check_trace("../trz/small/boundint-base-1.trz",grid,myparticles,densities,accelerations);
// TRAZE ACC COMPUTATION
/*
    //check_trace("../trz/small/densinc-base-1.trz",grid,particles,densities,accelerations);
...
// TRAZE  PARTICLE MOTION
/*
    load_trace("../trz/small/partcol-base-1.trz",grid,myparticles,densities,accelerations,initialValues);
    cout<<"\nNow motion\n";
    ...
    cout<<"\nNow check trace\n";
    check_trace("../trz/small/motion-base-1.trz",grid,myparticles,densities,accelerations);
    */
    write_to_file(argv[3],myparticles,initialValues);
